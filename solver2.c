 ...
 *
    int total_loop = 0;
    int empty_loop = 0;
    if (total_loop % 100000 == 0)
        printf("loop count for thread %d, is: %d, proc is: %d, queue size is %d. empty loop is:%d\n",
                omp_get_thread_num(), total_loop, t_working, t_size, empty_loop);
 */
